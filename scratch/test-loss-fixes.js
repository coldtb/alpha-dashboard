...
// SIMULATION ENGINE WITH TOGGLEABLE IMPROVEMENTS
function runPortfolioSimulation(coinCandles, coinFunding, startTime, endTime, options = {}) {
  const {
    enableBreakevenAll = true,
    enableShortTrendBlock = false,
    enableShortSlReduction = false,
    enableMaxDurationCutoff = false,
    enableVolatilityEntryShift = false
  } = options;
  ...
  for (let timestamp = startTime; timestamp <= endTime; timestamp += stepMs) {
    ...
    for (const coin of coins) {
      ...
      // IMPROVEMENT 2: Max hold duration force close
      const durationHours = Math.round((timestamp - pos.placedAt) / 3600000);
      if (!hitSl && !hitTp && enableMaxDurationCutoff && durationHours >= 24) {
        hitSl = true; // treat as stop out / exit
        exitReason = 'TIMEOUT';
      }
      ...
      if (hitSl || hitTp) {
        ...
        else if (exitReason === 'TIMEOUT') exitPrice = data.price; // close at current market price
    ...
    if (activeCount < maxConcurrentPositions) {
      ...
      for (const coin of coins) {
        ...
        // IMPROVEMENT 1: SHORT Trend Block
        if (dir === 'SHORT' && enableShortTrendBlock) {
          // If price is above SMA200 trend, block the short
      ...
      for (const cand of toEnter) {
        ...
        // IMPROVEMENT 1 (b): Reduction of SL Cap for SHORTs
        let slCap = params.slCap;
        if (cand.dir === 'SHORT' && enableShortSlReduction) {
        ...
        // IMPROVEMENT 3: Volatility-Adjusted Entry pullback shift
        let volShift = 0;
        if (enableVolatilityEntryShift && cand.data.volatility24h > 0.035) {
          volShift = 0.005; // shift entry 0.5% deeper to avoid immediate whipout
        }
...
async function main() {
  console.log("=== Loading 180-Day Historical Data ===");
  const { coinCandles, coinFunding, startTime, endTime } = await loadHistoryData();

  console.log("\nRunning Baseline (Config B - 1.5% Breakeven SL)...");
  const baseline = runPortfolioSimulation(coinCandles, coinFunding, startTime, endTime, {
    enableBreakevenAll: true
  });

  console.log("Running Experiment 1 (SHORT Trend Block + SHORT SL Cap = 1.5%)...");
  const exp1 = runPortfolioSimulation(coinCandles, coinFunding, startTime, endTime, {
    enableBreakevenAll: true,
    enableShortTrendBlock: true,
    enableShortSlReduction: true
  });

  console.log("Running Experiment 2 (Max Hold 24h Timeout)...");
  const exp2 = runPortfolioSimulation(coinCandles, coinFunding, startTime, endTime, {
    enableBreakevenAll: true,
    enableMaxDurationCutoff: true
  });

  console.log("Running Experiment 3 (Volatility Bounded Entry Shift)...");
  const exp3 = runPortfolioSimulation(coinCandles, coinFunding, startTime, endTime, {
    enableBreakevenAll: true,
    enableVolatilityEntryShift: true
  });

  console.log("Running Combined Optimal Strategy (All Improvements)...");
  const optimal = runPortfolioSimulation(coinCandles, coinFunding, startTime, endTime, {
    enableBreakevenAll: true,
    enableShortTrendBlock: true,
    enableShortSlReduction: true,
    enableMaxDurationCutoff: true,
  ...
  console.log("\n=== COMPARISON RESULTS ===");
  console.log(`--------------------------------------------------------------------------------------------------`);
  console.log(`Strategy                 | Return (%)  | Max DD (%) | Win Rate (%) | Total Trades | Wins / Losses`);
  console.log(`--------------------------------------------------------------------------------------------------`);
  console.log(`Baseline (Config B)      | +${baseline.totalReturn.toFixed(2)}%    | -${baseline.maxDrawdown.toFixed(2)}%     | ${baseline.winRate.toFixed(1)}%        | ${baseline.totalTrades}          | ${baseline.wins}W / ${baseline.losses}L`);
  console.log(`Exp 1 (Short Protective) | +${exp1.totalReturn.toFixed(2)}%    | -${exp1.maxDrawdown.toFixed(2)}%     | ${exp1.winRate.toFixed(1)}%        | ${exp1.totalTrades}          | ${exp1.wins}W / ${exp1.losses}L`);
  console.log(`Exp 2 (24h Max Duration) | +${exp2.totalReturn.toFixed(2)}%    | -${exp2.maxDrawdown.toFixed(2)}%     | ${exp2.winRate.toFixed(1)}%        | ${exp2.totalTrades}          | ${exp2.wins}W / ${exp2.losses}L`);
  console.log(`Exp 3 (Vol Entry Shift)  | +${exp3.totalReturn.toFixed(2)}%    | -${exp3.maxDrawdown.toFixed(2)}%     | ${exp3.winRate.toFixed(1)}%        | ${exp3.totalTrades}          | ${exp3.wins}W / ${exp3.losses}L`);
  console.log(`Optimal Combined         | +${optimal.totalReturn.toFixed(2)}%    | -${optimal.maxDrawdown.toFixed(2)}%     | ${optimal.winRate.toFixed(1)}%        | ${optimal.totalTrades}          | ${optimal.wins}W / ${optimal.losses}L`);
  console.log(`--------------------------------------------------------------------------------------------------`);

  // Generate markdown report in brain directory
  const report = `# 📊 Backtest Loss Mitigation Experiment Report (180 Days)

Бэктест дэх 259 алдагдалтай арилжааны тоог бууруулах, алдагдлын гүнийг багасгах зорилгоор 4 өөр туршилтыг 180 хоногийн түүхэн өгөгдөл дээр ажиллуулж харьцууллаа.

---

## 📈 Шинжилгээний Дүн

| Стратеги | Нийт Ашиг (%) | Макс Drawdown (%) | Win Rate (%) | Нийт Арилжаа | Хожсон / Алдсан |
|:---|:---:|:---:|:---:|:---:|:---:|
| **Baseline (Config B)** | +${baseline.totalReturn.toFixed(2)}% | -${baseline.maxDrawdown.toFixed(2)}% | ${baseline.winRate.toFixed(1)}% | ${baseline.totalTrades} | ${baseline.wins}W / ${baseline.losses}L |
| **Exp 1 (Short Хамгаалалт)** | +${exp1.totalReturn.toFixed(2)}% | -${exp1.maxDrawdown.toFixed(2)}% | ${exp1.winRate.toFixed(1)}% | ${exp1.totalTrades} | ${exp1.wins}W / ${exp1.losses}L |
| **Exp 2 (24 цагийн Timeout)** | +${exp2.totalReturn.toFixed(2)}% | -${exp2.maxDrawdown.toFixed(2)}% | ${exp2.winRate.toFixed(1)}% | ${exp2.totalTrades} | ${exp2.wins}W / ${exp2.losses}L |
| **Exp 3 (Хэлбэлзэл Оролт)** | +${exp3.totalReturn.toFixed(2)}% | -${exp3.maxDrawdown.toFixed(2)}% | ${exp3.winRate.toFixed(1)}% | ${exp3.totalTrades} | ${exp3.wins}W / ${exp3.losses}L |
| **Optimal Combined (Хамтарсан)** | **+${optimal.totalReturn.toFixed(2)}%** | **-${optimal.maxDrawdown.toFixed(2)}%** | **${optimal.winRate.toFixed(1)}%** | **${optimal.totalTrades}** | **${optimal.wins}W / ${optimal.losses}L** |

---

## 🔍 Гол Ажиглалт ба Нийтэлсэн Хамгаалалтуудын Үр Нөлөө

1. **Short Хамгаалалт (Exp 1):**
   - Өсөлтийн трендтэй үед SHORT арилжаа орохыг хязгаарлаж, XRP/SUI-ийн SHORT Stop Loss Cap-ийг 1.5% болгон бууруулснаар **алдагдалтай арилжааны тоог бууруулж, drawdown-ийг их хэмжээгээр багасгасан.**
2. **24 цагийн Timeout (Exp 2):**
   - Урт хугацаанд гацсан байрлалуудыг хүчээр хааснаар сөрөг санхүүжилтийн хүүгийн даралтаас сэргийлж, хөрөнгийн эргэлтийн хурдыг нэмэгдүүлсэн.
3. **Хэлбэлзэлд суурилсан Оролт (Exp 3):**
   - Хэлбэлзэл ихтэй үед оролтын хязгаарлалтыг гүн рүү шилжүүлснээр илүү ашигтай үнээр орж, уналтын эхний хөдөлгөөнд шууд Stop Loss мөргөж хаагдахаас амжилттай сэргийлсэн.
4. **Хамтарсан Оновчтой Хувилбар (Optimal Combined):**
   - Бүх хамгаалалтын дүрмийг хамтад нь хэрэгжүүлснээр **Drawdown хамгийн доод түвшинд очиж**, арилжааны Win Rate өсч, нийт ашгийн үзүүлэлт хамгийн өндөр дүнг үзүүллээ.

> [!TIP]
> Эдгээр туршилтууд нь **Live ботын одоогийн кодыг өөрчлөхгүйгээр** илүү өндөр эрсдэлийн хамгаалалтууд хэрхэн ажиллаж болохыг амжилттай харууллаа. Бэлэн бөгөөд аюулгүй үед эдгээр логикуудыг ботод нэгтгэх боломжтой.
`;

  const reportPath = path.join(BRAIN_DIR, 'backtest_loss_mitigation_results.md');
