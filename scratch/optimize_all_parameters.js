...
async function runBacktest(coinSymbol, candles, fundingMap, maxTpPct, maxSlPct, trendPeriod) {
  let balance = 10000;
  let position = null;
...
async function optimizeCoin(coinSymbol, days = 180) {
  console.log(`\n==========================================`);
  console.log(`Optimizing ${coinSymbol} (${days} days snapshot)...`);
  console.log(`==========================================`);
  ...
  console.log(`Successfully loaded ${candles.length} candles and ${fundingHistory.length} funding records.`);
  ...
  for (const tp of tpOpts) {
    for (const sl of slOpts) {
      for (const trend of trendOpts) {
        const r = await runBacktest(coinSymbol, candles, fundingMap, tp, sl, trend);
  ...
  console.log(`\n=== TOP 3 CONFIGURATIONS FOR ${coinSymbol} ===`);
  results.slice(0, 3).forEach((r, idx) => {
    ...
    console.log(`  Calmar Ratio: ${r.calmar.toFixed(2)}\n`);
  });
...
async function main() {
  const coins = ["XRP", "SUI", "HYPE"];
  const bestConfigs = {};
  for (const coin of coins) {
    bestConfigs[coin] = await optimizeCoin(coin, 180);
  }
}
