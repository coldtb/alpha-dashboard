...
async function loadHistoryData() {
  ...
  const activeCoins = ['BTC', 'XRP', 'SUI', 'HYPE'];
...
function runSimulation(coinCandles, coinFunding, startTime, endTime, mode, enableCooldown) {
  ...
  const hypeParams = { tpCap: 0.0075, slCap: 0.015, trendPeriod: 24 };
  const coinsToSimulate = mode === 'HYPE_ONLY' ? ['HYPE'] : ['BTC', 'XRP', 'SUI', 'HYPE'];
  const maxConcurrent = mode === 'HYPE_ONLY' ? 1 : 2;

  // Alt TP config mapping
  const coinParams = {
    BTC:  { tpCap: 0.02,   slCap: 0.015, trendPeriod: 200, cooldownEnabled: false },
    XRP:  { tpCap: 0.0075, slCap: 0.015, trendPeriod: 50,  cooldownEnabled: false },
    SUI:  { tpCap: 0.0075, slCap: 0.015, trendPeriod: 200, cooldownEnabled: enableCooldown },
    HYPE: { tpCap: 0.0075, slCap: 0.015, trendPeriod: 24,  cooldownEnabled: enableCooldown },
  };

  const pointers = {};
  for (const coin of coinsToSimulate) {
  ...
  for (let timestamp = startTime; timestamp <= endTime; timestamp += stepMs) {
    ...
    for (const coin of coinsToSimulate) {
      ...
      if (hitSl || hitTp) {
        ...
        const margin = Math.min(balance * 0.95, 50000) / maxConcurrent;
        ...
        if (coin === 'HYPE' && enableCooldown) {
          if (netRet > 0) {
            consecutiveLosses = 0;
          } else {
            consecutiveLosses++;
            if (consecutiveLosses >= 2) {
              cooldownUntil = timestamp + 24 * 3600000;
              consecutiveLosses = 0;
    ...
    if (activeCount < maxConcurrent) {
      ...
      for (const coin of coinsToSimulate) {
        ...
        if (coin === 'HYPE' && enableCooldown && timestamp < cooldownUntil) continue;
...
async function run() {
  ...
  console.log("\n--- Running Baseline (All 4 Coins, No Cooldown) ---");
  const baseline = runSimulation(coinCandles, coinFunding, startTime, endTime, 'ALL_4_COINS', false);
  
  console.log("\n--- Running Experiment 1 (HYPE Only, Cooldown Enabled) ---");
  const hypeCooldown = runSimulation(coinCandles, coinFunding, startTime, endTime, 'HYPE_ONLY', true);

  console.log("\n--- Running Experiment 2 (HYPE Only, Cooldown Disabled) ---");
  const hypeNoCooldown = runSimulation(coinCandles, coinFunding, startTime, endTime, 'HYPE_ONLY', false);

  const report = `# 📊 HYPE-Only Strategy Backtest Report

Энэхүү тайлан нь зөвхөн **HYPE** коин дээр арилжаа хийдэг нэг коины стратегийг (Single Coin Strategy) олон коины үндсэн багцтай харьцуулж харуулна.

## 📈 Багцын Нийт Харьцуулалт

| Үзүүлэлт | Үндсэн багц (4 Coins, Cooldown-гүй) | HYPE-Only (Cooldown-той) | HYPE-Only (Cooldown-гүй) |
| :--- | :---: | :---: | :---: |
| **Багцын Нийт Ашиг (%)** | +${baseline.totalReturn.toFixed(2)}% | +${hypeCooldown.totalReturn.toFixed(2)}% | +${hypeNoCooldown.totalReturn.toFixed(2)}% |
| **Максимум Drawdown (%)** | -${baseline.maxDrawdown.toFixed(2)}% | -${hypeCooldown.maxDrawdown.toFixed(2)}% | -${hypeNoCooldown.maxDrawdown.toFixed(2)}% |
| **Нийт Арилжааны тоо** | ${baseline.totalTrades} | ${hypeCooldown.totalTrades} | ${hypeNoCooldown.totalTrades} |
| **Win Rate (%)** | ${baseline.winRate.toFixed(1)}% | ${hypeCooldown.winRate.toFixed(1)}% | ${hypeNoCooldown.winRate.toFixed(1)}% |

---

## 🔍 Дүгнэлт
- **HYPE-Only хувилбарын ашиг ба эрсдэл:** Зөвхөн HYPE дээр ажиллахад нийт ашиг болон drawdown хэрхэн өөрчлөгдсөнийг харна уу. Нэг коинд 100% margin төвлөрүүлж байгаа тул ашиг өсөх боловч нэг коины эрсдэл (single asset risk) болон drawdown нэмэгдсэн байж болно.
- **Хөрөнгийн удирдлага:** 4 коинтой багц нь эрсдэлийг хувааж (diversification) өгдөг бол 1 коинтой багц нь илүү савлагаатай байж болно.
`;

  const reportPath = path.join(BRAIN_DIR, 'hype_only_comparison_report.md');
