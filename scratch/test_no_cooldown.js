...
function runSimulation(coinCandles, coinFunding, startTime, endTime, enableCooldown) {
...
async function run() {
  ...
  console.log("\n--- Running Baseline (Cooldown Enabled for SUI/HYPE) ---");
  const baseline = runSimulation(coinCandles, coinFunding, startTime, endTime, true);
  
  console.log("\n--- Running Experiment (Cooldown Disabled for ALL Coins) ---");
  const noCooldown = runSimulation(coinCandles, coinFunding, startTime, endTime, false);

  const coinBreakdown = coins.map(coin => {
    const coinTrades = noCooldown.trades.filter(t => t.coin === coin);
  ...
  const report = `# 📊 Cooldown Filter Comparison Report

Энэхүү тайлан нь дараалсан 2 алдагдлын дараа 24 цаг арилжаа хийхийг хориглодог Cooldown шүүлтүүрийг идэвхтэй байлгах болон идэвхгүй болгох үеийн багцын үр дүнг харьцуулна.

## 📈 Багцын Нийт Харьцуулалт

| Үзүүлэлт | Cooldown Идэвхтэй (Baseline) | Cooldown Байхгүй (Experiment) | Өөрчлөлт |
| :--- | :---: | :---: | :---: |
| **Багцын Нийт Ашиг (%)** | +\${baseline.totalReturn.toFixed(2)}% | +\${noCooldown.totalReturn.toFixed(2)}% | **\${(noCooldown.totalReturn - baseline.totalReturn).toFixed(2)}%** |
| **Максимум Drawdown (%)** | -\${baseline.maxDrawdown.toFixed(2)}% | -\${noCooldown.maxDrawdown.toFixed(2)}% | **\${(baseline.maxDrawdown - noCooldown.maxDrawdown).toFixed(2)}%** |
| **Нийт Арилжааны тоо** | \${baseline.totalTrades} | \${noCooldown.totalTrades} | **\${noCooldown.totalTrades - baseline.totalTrades}** |
| **Win Rate (%)** | \${baseline.winRate.toFixed(1)}% | \${noCooldown.winRate.toFixed(1)}% | **\${(noCooldown.winRate - baseline.winRate).toFixed(1)}%** |

---

## 🪙 Cooldown-гүй үеийн Коин тус бүрийн Нарийвчилсан Үр Дүн (Per-Coin Breakdown)
...
## 🔍 Дүгнэлт
- **Алдагдлын Cooldown-ы ач холбогдол:** Cooldown шүүлтүүрийг унтраахад арилжааны тоо хэрхэн нэмэгдсэн, үүнийг дагаад ашиг өссөн эсвэл дараалсан алдагдлуудад өртөж drawdown ихэссэн эсэхийг дээрх хүснэгтээс харьцуулна уу.
`;

  const reportPath = path.join(BRAIN_DIR, 'cooldown_comparison_report.md');
  fs.writeFileSync(reportPath, report, 'utf8');
  console.log(`Report successfully written to ${reportPath}`);

  console.log("\n=== COIN BREAKDOWN (NO COOLDOWN) ===");
