...
function getDirection(coin, sma24, smaTrend, trendLockActive = true) {
  ...
  if (sma24 !== null) {
    if (dir === 'LONG' && (price < sma24 || price > sma24 * 1.03)) return 'SKIP';
    if (dir === 'SHORT' && (price > sma24 || price < sma24 * 0.97)) return 'SKIP';
  }
  return dir;
}
...
async function runBtcBacktest(candles, fundingMap, maxTpPct, maxSlPct, trendPeriod) {
  let balance = 10000;
  let position = null;
  ...
  for (let i = 200; i < candles.length; i++) {
    ...
    const direction = getDirection(coinData, sma24, smaTrend, true);
...
async function main() {
  console.log("Fetching BTC historical candles (180 days) for grid search...");
  ...
  console.log(`Successfully fetched ${candles.length} candles and ${fundingHistory.length} funding rates.`);

  const tpOpts = [0.02, 0.03, 0.04, 0.05];
  const slOpts = [0.015, 0.02, 0.025];
  const trendOpts = [50, 100, 150, 200];
  ...
  for (const tp of tpOpts) {
    for (const sl of slOpts) {
      for (const trend of trendOpts) {
        const r = await runBtcBacktest(candles, fundingMap, tp, sl, trend);
        const returnPct = ((r.balance - 10000) / 10000) * 100;
        const calmar = r.maxDd > 0 ? returnPct / r.maxDd : returnPct;
        results.push({ tp, sl, trend, balance: r.balance, returnPct, maxDd: r.maxDd, wr: r.wr, trades: r.total, calmar });
      }
    }
  }

  // Sort by Calmar Ratio (Profit / Drawdown efficiency)
  results.sort((a, b) => b.calmar - a.calmar);

  console.log("\n=== TOP 5 OPTIMAL BTC CONFIGURATIONS (Sorted by Profit/Drawdown ratio) ===");
  results.slice(0, 5).forEach((r, idx) => {
    console.log(`Rank #${idx + 1}:`);
    console.log(`  Parameters: TP Cap = ${(r.tp * 100).toFixed(1)}% | SL Cap = ${(r.sl * 100).toFixed(1)}% | Trend Lock = SMA${r.trend}`);
    console.log(`  Metrics: Final Balance = $${r.balance.toFixed(2)} | Return = +${r.returnPct.toFixed(2)}%`);
    console.log(`  Drawdown = -${r.maxDd.toFixed(2)}% | Win Rate = ${r.wr.toFixed(1)}% | Trades = ${r.trades}`);
    console.log(`  Calmar Ratio (Profit/Drawdown efficiency): ${r.calmar.toFixed(2)}\n`);
  });
}
