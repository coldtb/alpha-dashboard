...
// Dynamic direction logic based on asset type
function detectAutoDirectionDynamic(coin, sma24, sma100, maxDistancePct, isHypeCoin) {
  ...
  if (isHypeCoin) {
    ...
    // XRP: Mean-Reverting, Funding-Biased (Original logic)
...
async function runTest(coinSymbol, days) {
  ...
  function simulate() {
    ...
    for (let i = 100; i < candles.length; i++) {
      ...
      const direction = detectAutoDirectionDynamic(coinData, sma24, sma100, 0.05, isHype);
      ...
      if (direction !== 'SKIP') {
        ...
        const dynamicSizeFactor = isHype 
          ? Math.min(0.95, Math.max(0.15, 0.05 / volatility24h)) // Volatility sizing for HYPE
          : 0.95; // Fixed sizing for XRP
  ...
  const r = simulate();
  console.log(`[Dynamic Strategy] ${coinSymbol} (${days} Days) -> Balance: $${r.balance.toFixed(2)} | WR: ${r.wr.toFixed(1)}% | Max DD: ${r.maxDd.toFixed(1)}% | Trades: ${r.total}`);
}

async function main() {
  console.log("=== RUNNING ASSET-SPECIFIC DYNAMIC STRATEGY TESTS ===");
  await runTest("XRP", 180);
  await runTest("HYPE", 180);
  await runTest("HYPE", 90);
}
