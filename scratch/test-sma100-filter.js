...
// Improved detectAutoDirection with SMA100 trend filter
function detectAutoDirection(coin, sma24, sma100, maxDistancePct) {
  const funding = coin.funding || 0;
  const change24h = coin.change || 0;
  
  // Weight change24h more than funding to prevent shorting bull runs
  let score = 0;
  if (change24h > 2.0) score += 2;
  else if (change24h < -2.0) score -= 2;

  if (funding < -0.0002) score += 1;
  else if (funding > 0.0002) score -= 1;
  ...
  // Apply dual SMA filter (SMA24 + SMA100)
  const price = coin.price;
  if (dir === 'LONG') {
    // For LONG: Price must be above both SMA24 and SMA100
    if (price < sma24 || price < sma100) return 'SKIP';
    if (price > sma24 * (1 + maxDistancePct)) return 'SKIP';
  } else {
    // For SHORT: Price must be below both SMA24 and SMA100
    if (price > sma24 || price > sma100) return 'SKIP';
    if (price < sma24 * (1 - maxDistancePct)) return 'SKIP';
  }
...
async function runTest(coinSymbol, days) {
  ...
  function simulate(useImprovedFilter) {
    ...
    for (let i = 100; i < candles.length; i++) {
      ...
      for (let j = i - 24; j <= i; j++) {
        sumClose24 += parseFloat(candles[j].c);
      ...
      if (score >= config.minScore) {
        if (useImprovedFilter) {
          direction = detectAutoDirection(coinData, sma24, sma100, 0.05);
        } else {
  ...
  console.log(`[+SMA100 + Change]:  Final Balance: $${r2.balance.toFixed(2)} | Win Rate: ${r2.wr.toFixed(1)}% | Max DD: ${r2.maxDd.toFixed(1)}% | Trades: ${r2.total}`);
}
