...
function runOptimalSimulation(coinCandles, coinFunding, startTime, endTime, btcTpCap) {
  ...
  const coinParams = {
    BTC:  { tpCap: btcTpCap,  slCap: 0.015, trendPeriod: 200, cooldownEnabled: false },
...
async function run() {
  ...
  console.log("\n--- Running Baseline (BTC TP = 4%) ---");
  const res4 = runOptimalSimulation(coinCandles, coinFunding, startTime, endTime, 0.04);
  const btc4 = res4.trades.filter(t => t.coin === 'BTC');
  const btc4Wins = btc4.filter(t => t.pnl > 0).length;
  const btc4Losses = btc4.filter(t => t.pnl <= 0).length;

  console.log("\n--- Running Experiment (BTC TP = 3%) ---");
  const res3 = runOptimalSimulation(coinCandles, coinFunding, startTime, endTime, 0.03);
  const btc3 = res3.trades.filter(t => t.coin === 'BTC');
  const btc3Wins = btc3.filter(t => t.pnl > 0).length;
  const btc3Losses = btc3.filter(t => t.pnl <= 0).length;

  console.log("\n--- Running Experiment (BTC TP = 2%) ---");
  const res2 = runOptimalSimulation(coinCandles, coinFunding, startTime, endTime, 0.02);
  const btc2 = res2.trades.filter(t => t.coin === 'BTC');
  const btc2Wins = btc2.filter(t => t.pnl > 0).length;
  const btc2Losses = btc2.filter(t => t.pnl <= 0).length;

  console.log("\n--- 3-Way Comparison Finished ---");
  console.log(`4% TP: Return +${res4.totalReturn.toFixed(2)}%, Max DD -${res4.maxDrawdown.toFixed(2)}% | BTC Net PnL: $${btc4.reduce((sum, t) => sum + t.pnl, 0).toFixed(2)}`);
  console.log(`3% TP: Return +${res3.totalReturn.toFixed(2)}%, Max DD -${res3.maxDrawdown.toFixed(2)}% | BTC Net PnL: $${btc3.reduce((sum, t) => sum + t.pnl, 0).toFixed(2)}`);
  console.log(`2% TP: Return +${res2.totalReturn.toFixed(2)}%, Max DD -${res2.maxDrawdown.toFixed(2)}% | BTC Net PnL: $${btc2.reduce((sum, t) => sum + t.pnl, 0).toFixed(2)}`);

  // Calculate per-coin breakdown for BTC TP = 2%
  const coinBreakdown2 = coins.map(coin => {
    const coinTrades = res2.trades.filter(t => t.coin === coin);
    ...
    return { coin, total, wins, losses, winRate, pnl };
  });

  // Write comparison report
  const report = `# 📊 BTC TP 4% vs 3% vs 2% Comparison Report

Энэхүү тайлан нь BTC-ийн TP Cap (Take Profit)-ийг **4%**, **3%**, **2%** болгож бууруулахад багцын болон BTC-ийн үр дүнд хэрхэн нөлөөлснийг харуулна.

---

## 📈 Багцын Нийт Харьцуулалт

| Үзүүлэлт | BTC TP = 4% (Baseline) | BTC TP = 3% (Experiment 1) | BTC TP = 2% (Experiment 2) |
| :--- | :---: | :---: | :---: |
| **Багцын Нийт Ашиг (%)** | +${res4.totalReturn.toFixed(2)}% | +${res3.totalReturn.toFixed(2)}% | **+${res2.totalReturn.toFixed(2)}%** |
| **Максимум Drawdown (%)** | -${res4.maxDrawdown.toFixed(2)}% | -${res3.maxDrawdown.toFixed(2)}% | **-${res2.maxDrawdown.toFixed(2)}%** |
| **Нийт Арилжааны тоо** | ${res4.totalTrades} | ${res3.totalTrades} | **${res2.totalTrades}** |
| **Win Rate (%)** | ${res4.winRate.toFixed(1)}% | ${res3.winRate.toFixed(1)}% | **${res2.winRate.toFixed(1)}%** |

---

## 🪙 BTC-ийн Үзүүлэлтийн Харьцуулалт

| BTC Үзүүлэлт | BTC TP = 4% | BTC TP = 3% | BTC TP = 2% |
| :--- | :---: | :---: | :---: |
| **BTC Нийт Арилжаа** | ${btc4.length} | ${btc3.length} | **${btc2.length}** |
| **BTC Хожсон (Wins)** | ${btc4Wins} | ${btc3Wins} | **${btc2Wins}** |
| **BTC Алдсан (Losses)** | ${btc4Losses} | ${btc3Losses} | **${btc2Losses}** |
| **BTC Win Rate (%)** | ${((btc4Wins / (btc4.length || 1)) * 100).toFixed(1)}% | ${((btc3Wins / (btc3.length || 1)) * 100).toFixed(1)}% | **${((btc2Wins / (btc2.length || 1)) * 100).toFixed(1)}%** |
| **BTC Цэвэр Ашиг (PnL)** | $${btc4.reduce((sum, t) => sum + t.pnl, 0).toFixed(2)} | $${btc3.reduce((sum, t) => sum + t.pnl, 0).toFixed(2)} | **$${btc2.reduce((sum, t) => sum + t.pnl, 0).toFixed(2)}** |

---

## 📊 3. BTC TP = 2% үеийн Коин тус бүрийн Нарийвчилсан Үр Дүн (Per-Coin Breakdown)

BTC-ийн TP-ийг 2% болгож ажиллуулахад, бусад 3 коинтой хамт нэгэн зэрэг арилжаалагдсан үеийн коин тус бүрийн үзүүлэлт:

| Коин | Нийт Арилжаа | Хожсон (Wins) | Алдсан (Losses) | Win Rate (%) | Нийт Цэвэр Ашиг (PnL) |
| :--- | :---: | :---: | :---: | :---: | :---: |
${coinBreakdown2.map(c => `| **${c.coin}** | ${c.total} | ${c.wins} | ${c.losses} | ${c.winRate}% | $${c.pnl.toFixed(2)} |`).join('\n')}

---

## 🔍 Дүгнэлт
- **Хамтран ажиллах логик:** Энэхүү бэктест нь 4 коиныг багц хэлбэрээр (нэгэн зэрэг ажиллах дээд хязгаар maxConcurrentPositions = 2 буюу хамгийн ихдээ 2 коин зэрэг байрлал эзэмших) ажиллуулсан.
- **Үр дүн:** BTC-ийн TP-ийг 2% болгон бууруулахад багц дахь арилжааны эргэц илүү хурдан болж, зөвхөн BTC-ээс гадна бусад коинуудын ашигтай арилжаанд орох боломжийг суллаж өгсөн тул нийт багцын ашиг хамгийн өндөр хэмжээнд хүрсэн байна.
`;

  const reportPath = path.join(BRAIN_DIR, 'btc_tp_comparison.md');
