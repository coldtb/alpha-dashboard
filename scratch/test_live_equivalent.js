  ...
  for (let timestamp = startTime; timestamp <= endTime; timestamp += stepMs) {
    ...
    for (const coin of coins) {
      ...
      if (hitSl || hitTp) {
        ...
        const margin = (Math.min(balance * 0.95, 50000) / maxConcurrentPositions) * scale;
...
async function run() {
  ...
  console.log("\n--- Running Live-Equivalent Portfolio Backtest (4 Coins, Trailing & Pyramiding, Cooldown Disabled) ---");
  const sim = runSimulation(coinCandles, coinFunding, startTime, endTime, true);
  ...
  const report = `# 📊 Live-Equivalent Portfolio Backtest Report (4 Coins)

Энэхүү тайлан нь одоогийн live bot-ын тохиргоог 100% бодит захиалгын гүйцэтгэл (Honest Fills) болон Trailing SL & Pyramiding системтэйгээр 4 коин дээр бэктест хийсэн үр дүнг харуулна.

## 📈 Багцын Нийт Харьцуулалт (Summary)

| Үзүүлэлт | Live Bot Тохиргоотой Багц |
| :--- | :---: |
...
## 🔍 Дүгнэлт
- **Бодит Тэргүүлэл:** Live bot-ын одоогийн тохиргоогоор ажиллуулахад багцын ашиг болон эрсдэлийн бодит харьцаа ямар гарсаныг дээрх хүснэгтээс харна уу.
`;

  const reportPath = path.join(BRAIN_DIR, 'live_equivalent_report.md');
