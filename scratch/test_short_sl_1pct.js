...
function runSimulation(coinCandles, coinFunding, startTime, endTime, shortSlCap) {
  ...
  for (let timestamp = startTime; timestamp <= endTime; timestamp += stepMs) {
    ...
    if (activeCount < maxConcurrentPositions) {
      ...
      for (const cand of toEnter) {
        ...
        if (cand.dir === 'SHORT') {
          slCap = shortSlCap; // Override Stop Loss Cap on Shorts
        }
...
async function run() {
  ...
  console.log("\n--- Running Baseline (SHORT SL Cap = 1.5%) ---");
  const baseline = runSimulation(coinCandles, coinFunding, startTime, endTime, 0.015);
  
  console.log("\n--- Running Experiment (SHORT SL Cap = 2.0%) ---");
  const test2pct = runSimulation(coinCandles, coinFunding, startTime, endTime, 0.02);

  const report = `# 📊 SHORT Stop Loss Cap: 1.5% vs 2.0% Comparison

Энэхүү тайлан нь SHORT арилжааны Stop Loss-ийг суллах буюу **1.5%**-иос **2.0%** болгон нэмэгдүүлэхэд багцын болон арилжааны үр дүнд хэрхэн нөлөөлснийг харуулна.

## 📈 Багцын Нийт Харьцуулалт

| Үзүүлэлт | SHORT SL Cap = 1.5% (Baseline) | SHORT SL Cap = 2.0% (Experiment) | Өөрчлөлт |
| :--- | :---: | :---: | :---: |
| **Багцын Нийт Ашиг (%)** | +${baseline.totalReturn.toFixed(2)}% | +${test2pct.totalReturn.toFixed(2)}% | **${(test2pct.totalReturn - baseline.totalReturn).toFixed(2)}%** |
| **Максимум Drawdown (%)** | -${baseline.maxDrawdown.toFixed(2)}% | -${test2pct.maxDrawdown.toFixed(2)}% | **${(baseline.maxDrawdown - test2pct.maxDrawdown).toFixed(2)}%** |
| **Нийт Арилжааны тоо** | ${baseline.totalTrades} | ${test2pct.totalTrades} | **${test2pct.totalTrades - baseline.totalTrades}** |
| **Win Rate (%)** | ${baseline.winRate.toFixed(1)}% | ${test2pct.winRate.toFixed(1)}% | **${(test2pct.winRate - baseline.winRate).toFixed(1)}%** |

---

## 🪙 SHORT Арилжааны Нарийвчилсан Үр Дүн

| SHORT Үзүүлэлтүүд | SHORT SL Cap = 1.5% | SHORT SL Cap = 2.0% | Өөрчлөлт |
| :--- | :---: | :---: | :---: |
| **Нийт SHORT арилжаа** | ${baseline.trades.filter(t => t.dir === 'SHORT').length} | ${test2pct.trades.filter(t => t.dir === 'SHORT').length} | **${test2pct.trades.filter(t => t.dir === 'SHORT').length - baseline.trades.filter(t => t.dir === 'SHORT').length}** |
| **SHORT Хожсон (Wins)** | ${baseline.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length} | ${test2pct.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length} | **${test2pct.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length - baseline.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length}** |
| **SHORT Алдсан (Losses)** | ${baseline.trades.filter(t => t.dir === 'SHORT' && t.pnl <= 0).length} | ${test2pct.trades.filter(t => t.dir === 'SHORT' && t.pnl <= 0).length} | **${test2pct.trades.filter(t => t.dir === 'SHORT' && t.pnl <= 0).length - baseline.trades.filter(t => t.dir === 'SHORT' && t.pnl <= 0).length}** |
| **SHORT Win Rate (%)** | ${((baseline.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length / (baseline.trades.filter(t => t.dir === 'SHORT').length || 1)) * 100).toFixed(1)}% | ${((test2pct.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length / (test2pct.trades.filter(t => t.dir === 'SHORT').length || 1)) * 100).toFixed(1)}% | **${(((test2pct.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length / (test2pct.trades.filter(t => t.dir === 'SHORT').length || 1)) - (baseline.trades.filter(t => t.dir === 'SHORT' && t.pnl > 0).length / (baseline.trades.filter(t => t.dir === 'SHORT').length || 1))) * 100).toFixed(1)}%** |
| **SHORT Нийт PnL ($)** | $${baseline.trades.filter(t => t.dir === 'SHORT').reduce((sum, t) => sum + t.pnl, 0).toFixed(2)} | $${test2pct.trades.filter(t => t.dir === 'SHORT').reduce((sum, t) => sum + t.pnl, 0).toFixed(2)} | **$${(test2pct.trades.filter(t => t.dir === 'SHORT').reduce((sum, t) => sum + t.pnl, 0) - baseline.trades.filter(t => t.dir === 'SHORT').reduce((sum, t) => sum + t.pnl, 0)).toFixed(2)}** |

---

## 🔍 Дүгнэлт
- **Stop Loss-ийг суллахын нөлөөлөл:** SL-ийг 2.0% болгон өсгөхөд илүү олон арилжаа амжилттай болж Win Rate өсөх магадлалтай ч, алдагдалтай хаагдсан арилжааны сөрөг нөлөө (нэг алдахдаа илүү их алдах) өндөр болно. Үүний үр дүнд нийт ашиг болон drawdown-д хэрхэн нөлөөлснийг дээрх хүснэгтээс харна уу.
`;

  const reportPath = path.join(BRAIN_DIR, 'short_sl_comparison_report.md');
