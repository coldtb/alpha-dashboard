...
async function run() {
  ...
  const trades = runOptimalSimulation(coinCandles, coinFunding, startTime, endTime, 0.02); // 2% BTC TP

  const losses = trades.filter(t => t.pnl <= 0);
  const longLosses = losses.filter(t => t.dir === 'LONG');
  const shortLosses = losses.filter(t => t.dir === 'SHORT');

  const totalLongLossPnl = longLosses.reduce((sum, t) => sum + t.pnl, 0);
  const totalShortLossPnl = shortLosses.reduce((sum, t) => sum + t.pnl, 0);

  const avgLongLossPnl = totalLongLossPnl / (longLosses.length || 1);
  const avgShortLossPnl = totalShortLossPnl / (shortLosses.length || 1);

  console.log(`\n=== LOSS ANALYSIS BY DIRECTION ===`);
  console.log(`LONG Losses:`);
  console.log(`  Count: ${longLosses.length}`);
  console.log(`  Total Loss: $${totalLongLossPnl.toFixed(2)}`);
  console.log(`  Average Loss: $${avgLongLossPnl.toFixed(2)}`);
  
  console.log(`\nSHORT Losses:`);
  console.log(`  Count: ${shortLosses.length}`);
  console.log(`  Total Loss: $${totalShortLossPnl.toFixed(2)}`);
  console.log(`  Average Loss: $${avgShortLossPnl.toFixed(2)}`);

  console.log(`\nSummary:`);
  if (Math.abs(totalLongLossPnl) > Math.abs(totalShortLossPnl)) {
    console.log(`  LONG positions lost MORE money in total ($${totalLongLossPnl.toFixed(2)} vs $${totalShortLossPnl.toFixed(2)})`);
  } else {
    console.log(`  SHORT positions lost MORE money in total ($${totalShortLossPnl.toFixed(2)} vs $${totalLongLossPnl.toFixed(2)})`);
  }
}
